...
void Player::InitExplosion(SDL_Renderer* screen) {
    bool ret = explosion_.LoadImg("Threats/exp3.png", screen);
    if (ret) {
        explosion_.set_clip();
    }
}
...
void Player::CheckToMap(Map& map_data) {
    ...
    if (x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y) {
        if (x_val_ > 0) { 
            ...
            else if(val1 == FLYBOX || val2 == FLYBOX){
                map_data.tile[y1][x2] = 0;
                map_data.tile[y2][x2] = 0;
                RandomIncreaseBrain();
            }
...
void Player::PlayBulletHitSound() {
    Mix_PlayChannel(-1, g_bullet_hit_sound, 0);
}

void Player::PlayEatBrainSound() {
    Mix_PlayChannel(-1, g_eat_brain_sound, 0);
}

void Player::PlayHitThreatSound() {
    Mix_PlayChannel(-1, g_hit_threat_sound, 0);
}

void Player::PlayHitBoomSound() {
    Mix_PlayChannel(-1, g_hit_boom_sound, 0);
}
